...
func makeScanResult(projectID, taskType, summary string) EpisodicScanResult {
	return EpisodicScanResult{
		ID:        uuid.New(),
		ProjectID: projectID,
...
func TestClusterByPattern_GroupsByProjectAndTaskType(t *testing.T) {
	...
	assert.Len(t, clusters, 1) // only projA/debug has >= 2
...
func TestBacktest_SufficientSize(t *testing.T) {
	cl := PatternCluster{
		Episodics: make([]EpisodicScanResult, 5),
	}
...
func TestBacktest_InsufficientSize(t *testing.T) {
	cl := PatternCluster{
		Episodics: make([]EpisodicScanResult, 2),
	}
...
func TestBacktest_ExactlyMinSize(t *testing.T) {
	cl := PatternCluster{
		Episodics: make([]EpisodicScanResult, 3),
	}
	assert.Equal(t, 1.0, backtest(cl, 3))
}
