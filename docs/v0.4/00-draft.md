# VatBrain v0.4 — 评测驱动精修 + 价值证明

> 状态：**已定案**（决策点 1-4 已确认，2026-08-10；待实施）
>
> 日期：2026-08-10
>
> 前置阅读：
...
- **域选择（迁移信号，2026-08-10 已定案）**：
  1. **同题不同 seed**（先做，成本最低）：OmniMath 改 seed 重出题，解法可复用 —— 最快出 trend
  2. **代码修补场景**（后做）：同实体反复修改/出错（SWE-Bench 类，本机无 docker → 构造轻量修补任务集）—— 最贴近 pitfall 定位
...
## 11. 决策定案（2026-08-10，用户确认）

| # | 决策点 | 定案 |
|---|---|---|
| 1 | v0.4 定位 | ✅ **评测驱动精修 + 价值证明**（DX 收尾留 P1 小项） |
| 2 | Agent 轨二轮域 | ✅ **同题不同 seed（先）+ 代码修补场景（后）** |
| 3 | 时序深入版范围 | ✅ **最小深入**：加 `occurred_at` 列 + 检索时间排序/过滤，不动 provider 协议 |
| 4 | 反事实推理 | ✅ **P2，等数据**（Agent 轨二轮后再评估） |
| 5 | 准出目标幅度 | ⏳ 默认保留（表 7 数值），实施中按真实基线复核 |

---

*定案版本。按 §8 验收节奏 P0 → P1 顺序实施，范围随证据移动。*
